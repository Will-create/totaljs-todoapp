NEWSCHEMA('Todos', function(schema) {
    schema.action('list', {
        ...
        action: function($) {
            ...
            var builder = DB().find('nosql/todos'); // build our query
            builder.where('isremoved', false);

            // enable searching via request query parameter
            $.query.search && builder.search('search', $.query.search);
            builder.fields('id,name,isdone,description,dtcreated');
    ...
    schema.action('read', {
        ...
        action: function($) {
            var params = $.params;
            DB().read('nosql/todos').fields('id,name,isdone,description,dtcreated').id(params.id).where('isremoved', false).error(404).callback($.callback);
        }
    });
